...
var { loadPlayerData } = require('./persistence/saveManager.js');
var { autoSaveManager } = require('./persistence/saveManager.js');
...
// If roomType is specified, it overrides the config's default gameMode.
function loadMode(name, roomType) {
    // Map room types to their corresponding game modes
    if (roomType === 'pvp') {
        name = 'pvp';
    } else if (roomType === 'pve') {
        name = 'idleclicker';
    }
...
exports.getRoom = function (sig, size, roomType, pvpTier) {
    return new Room(sig, size, roomType, pvpTier);
};

class Room {
    constructor(sig, size, roomType, pvpTier) {
        this.sig = sig;
        this.size = size;
        this.roomType = roomType || 'pve';
        this.pvpTier  = pvpTier  || 1;
        this.clientList = {};
        ...
        this.game = new Game(this.clientList, this.playerList, this.world, this.engine, this.sig, this.roomType, pvpTier);
    }
    join(clientID) {
    ...
    async spawnPlayer(clientID) {
        var player = this.world.createNewPlayer(clientID);
        // Tag the player with the room type so the compressor can choose
        // the right wire format (idleplayer vs pvpplayer) without needing
        // to know which room the entity lives in.
        player.roomType = this.roomType;
        this.playerList[clientID] = player;
        
        // Load player data from database if authenticated
        var client = messenger.getClient(clientID);
        if (client && client.request && client.request.session) {
            var user = client.request.session.passport ? client.request.session.passport.user : null;
            if (user && user.avatarUrl) {
                player.avatarUrl = user.avatarUrl;
            }
            if (user && user.userId && user.authProvider) {
                console.log('Loading player data for ' + user.username + ' (' + user.authProvider + ':' + user.userId + ')');
                var loaded = await loadPlayerData(player, user.userId, user.authProvider);
                if (loaded) {
                    console.log('Player data loaded successfully');
                    // Register for auto-save
                    autoSaveManager.register(clientID, player, user.userId, user.authProvider);
                } else {
...
class Game {
    constructor(clientList, playerList, world, engine, roomSig, roomType, pvpTier) {
        ...
        this.mode = loadMode(c.gameMode, this.roomType);
    }
